...
class EncoderDecoderTransformer(nn.Module):
    def __init__(self, lookback, num_features, horizon, d_model=64, num_heads=4, d_ff=128, num_layers=2, dropout_rate=0.1):
        ...
        self.dec_attn = nn.ModuleList([nn.MultiheadAttention(embed_dim=d_model, num_heads=num_heads, dropout=dropout_rate, batch_first=True) for _ in range(num_layers)])
        self.norm1_dec = nn.ModuleList([nn.LayerNorm(d_model) for _ in range(num_layers)])
        self.cross_attn = nn.ModuleList([nn.MultiheadAttention(embed_dim=d_model, num_heads=num_heads, dropout=dropout_rate, batch_first=True) for _ in range(num_layers)])
...
output_json_filename = "03_tfm_encdec_pytorch_results.json"
results_data = {
    "model_name": "03_tfm_encdec_pytorch",
    "seeds": {},
    "summary": {}
}
all_seed_metrics = []
...
print(f"Starting Automated {len(SEEDS)}-Seed Loop for 03_tfm_encdec_pytorch in PyTorch...")
...
for seed_idx, SEED in enumerate(SEEDS, 1):
    ...
    # Build Model
    model = EncoderDecoderTransformer(lookback=LOOKBACK, num_features=X_train_scaled.shape[1], horizon=HORIZON, d_model=128, num_heads=8, d_ff=512, num_layers=1, dropout_rate=0.05).to(device)
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    results_data["total_parameters"] = total_params

    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=0.0003974934648296826, weight_decay=1.7727275186238125e-06)
    ...
    if best_val_loss < best_overall_val_loss and best_model_weights is not None:
        ...
        best_seed_id = SEED
        torch.save(best_model_weights, f"03_tfm_encdec_pytorch_best.pt")
        results_data["best_seed"] = int(SEED)
        print(f"  [Checkpoint] New overall best model saved from SEED {SEED} (Val Loss: {best_val_loss:.6f}) -> 03_tfm_encdec_pytorch_best.pt")
...
np.savez_compressed(f"03_tfm_encdec_pytorch_predictions.npz", **all_predictions)
print(f"Successfully saved all seed predictions to 03_tfm_encdec_pytorch_predictions.npz")

print(f"\n======================================================================")
print(f"FINAL SUMMARY ACROSS {len(SEEDS)} SEEDS — 03_tfm_encdec_pytorch")
print(f"======================================================================")
summary_dict = {}
