...
def run_benchmark():
    ...
    print(f"Starting Automated {len(SEEDS)}-Seed Loop for 06_tfm_ptst_pytorch in PyTorch...")
    ...
    for seed_idx, SEED in enumerate(SEEDS, 1):
        ...
        # Build Model
        model = PatchTSTModel(lookback=LOOKBACK, num_features=X_train_scaled.shape[1], horizon=HORIZON, patch_len=16, stride=8, d_model=128, num_heads=8, d_ff=256, num_layers=2, dropout_rate=0.2).to(device)
        total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        results_data["total_parameters"] = total_params

        criterion = nn.MSELoss()
        optimizer = optim.Adam(model.parameters(), lr=0.000742896080085223, weight_decay=4.138104584568934e-06)
