...
# ==============================================================================
# Multi-Seed Benchmark for Model 28: Crossformer
...
# ---------------------------------------------------------
# 3. Model Architecture: Crossformer
# ---------------------------------------------------------
...
class Crossformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_d = x.transpose(1, 2)
        x_seg = x_d.unfold(dimension=2, size=self.seg_len, step=self.seg_len)

        h = self.seg_proj(x_seg) + self.pos_time + self.pos_dim

        for layer in self.layers:
            h = layer(h)

        h_target = h[:, self.target_idx, :, :]
        h_flat = h_target.reshape(B, -1)

        forecast = self.head(h_flat)
        return forecast
...
CONFIG = {
    ...
    "num_layers": 3,
    "dropout": 0.2,
    "learning_rate": 0.0002294005425118865,
    "weight_decay": 3.859710600120065e-05,
    "batch_size": 64,
    "epochs": 200,
    "patience": 15}
