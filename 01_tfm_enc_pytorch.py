...
output_json_filename = "01_tfm_enc_pytorch_results.json"
results_data = {
    "model_name": "01_tfm_enc_pytorch",
    "seeds": {},
    "summary": {}
}
all_seed_metrics = []
...
def run_benchmark():
    ...
    print(f"Starting Automated {len(SEEDS)}-Seed Loop for 01_tfm_enc_pytorch in PyTorch...")
    ...
    for seed_idx, SEED in enumerate(SEEDS, 1):
        ...
        model = EncoderOnlyTransformer(lookback=LOOKBACK, num_features=X_train_scaled.shape[1], horizon=HORIZON, d_model=128, num_heads=2, d_ff=512, num_layers=1, dropout_rate=0.1).to(device)
        total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        results_data["total_parameters"] = total_params

        criterion = nn.MSELoss()
        optimizer = optim.Adam(model.parameters(), lr=0.00047330128560354453, weight_decay=0.00015039504742303124)
        ...
        if best_val_loss < best_overall_val_loss and best_model_weights is not None:
            ...
            best_seed_id = SEED
            torch.save(best_model_weights, f"01_tfm_enc_pytorch_best.pt")
            results_data["best_seed"] = int(SEED)
            print(f"  [Checkpoint] New overall best model saved from SEED {SEED} (Val Loss: {best_val_loss:.6f}) -> 01_tfm_enc_pytorch_best.pt")
    ...
    np.savez_compressed(f"01_tfm_enc_pytorch_predictions.npz", **all_predictions)
    print(f"Successfully saved all seed predictions to 01_tfm_enc_pytorch_predictions.npz")

    print(f"\n======================================================================")
    print(f"FINAL SUMMARY ACROSS {len(SEEDS)} SEEDS — 01_tfm_enc_pytorch")
    print(f"======================================================================")
    summary_dict = {}
