...
output_json_filename = "15_timemachine_baseline_pytorch_results.json"
results_data = {
    "model_name": "15_timemachine_baseline_pytorch",
    "seeds": {},
    "summary": {}
}
...
print(f"Starting Automated {len(SEEDS)}-Seed Loop for 15_timemachine_baseline_pytorch in PyTorch...")
...
for seed_idx, SEED in enumerate(SEEDS, 1):
    ...
    model = TimeMachineModel(lookback=LOOKBACK, num_features=X_train_scaled.shape[1], horizon=HORIZON, d_model=32, d_state=8, num_layers=1, dropout_rate=0.2).to(device)
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    results_data["total_parameters"] = total_params
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=0.0004994771893035688, weight_decay=8.120449103577722e-05)
    ...
    if best_val_loss < best_overall_val_loss and best_model_weights is not None:
        ...
        best_seed_id = SEED
        torch.save(best_model_weights, f"15_timemachine_baseline_pytorch_best.pt")
        results_data["best_seed"] = int(SEED)
        print(f"  [Checkpoint] New overall best model saved from SEED {SEED} (Val Loss: {best_val_loss:.6f}) -> 15_timemachine_baseline_pytorch_best.pt")
...
np.savez_compressed(f"15_timemachine_baseline_pytorch_predictions.npz", **all_predictions)
print(f"Successfully saved all seed predictions to 15_timemachine_baseline_pytorch_predictions.npz")

print(f"\n======================================================================")
print(f"FINAL SUMMARY ACROSS {len(SEEDS)} SEEDS — 15_timemachine_baseline_pytorch")
print(f"======================================================================")
summary_dict = {}
...
print(f"\nFinished running all {len(SEEDS)} SEEDs for 15_timemachine_baseline_pytorch!")
