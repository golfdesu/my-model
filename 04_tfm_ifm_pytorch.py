...
def run_benchmark():
    ...
    for seed_idx, SEED in enumerate(SEEDS, 1):
        ...
        # Build Model
        model = InformerModel(lookback=LOOKBACK, num_features=X_train_scaled.shape[1], horizon=HORIZON, d_model=128, num_heads=2, d_ff=256, num_layers=3, dropout_rate=0.1).to(device)
        total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        results_data["total_parameters"] = total_params

        criterion = nn.MSELoss()
        optimizer = optim.Adam(model.parameters(), lr=0.0006846904418242568, weight_decay=2.376782464217708e-06)
