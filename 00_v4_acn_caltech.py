...
# ==============================================================================
# 1. Dataset Loading & Preprocessing (ACN Caltech with Weather)
# ==============================================================================
...
# Keep the validated weather features for the with-weather benchmark.
...
print(f"  -> EEO Subspace Partition: {len(endo_indices)} Endogenous Features, {0 if exo_indices is None else len(exo_indices)} Exogenous Weather Features")
...
# Hyperparameters (Matching Seq2Seq optimal baseline 03 on Caltech with weather)
D_MODEL             = 64
NUM_HEADS           = 4
D_FF                = 128
NUM_LAYERS          = 2
...
output_archive_dir = "outputs/acn_caltech/00_tfm_custom_v4"
results_data = {
...
for seed_idx, SEED in enumerate(SEEDS, 1):
    ...
    for step in steps_to_eval:
        ...
        output_lines.append(f"  Overall RÂ²    : {m['r2']:.4f}")
...
print(f"Successfully saved all seed predictions to {predictions_filename}")

print(f"\n======================================================================")
print(f"FINAL SUMMARY ACROSS {len(SEEDS)} SEEDS â€” 00_tfm_custom_pytorch")
print(f"======================================================================")
summary_dict = {}
...
for k in metric_keys:
    ...
    if vals:
        ...
        print(f"  {k.upper():<22}: {mu:.4f} Â± {sigma:.4f}")
...
# Automatically archive artifacts to the dataset-specific v4 output directory.
import shutil
os.makedirs(output_archive_dir, exist_ok=True)
for fname in [output_json_filename, best_model_filename, predictions_filename]:
    if os.path.exists(fname):
        shutil.copy(fname, os.path.join(output_archive_dir, fname))
print(f"Successfully archived all artifacts to {output_archive_dir}/")
