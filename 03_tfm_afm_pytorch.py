...
output_json_filename = "03_tfm_afm_pytorch_results.json"
results_data = {
    "model_name": "03_tfm_afm_pytorch",
    "seeds": {},
    "summary": {}
}
all_seed_metrics = []
...
print(f"Starting Automated {len(SEEDS)}-Seed Loop for 03_tfm_afm_pytorch in PyTorch...")
...
for seed_idx, SEED in enumerate(SEEDS, 1):
    ...
    # Build Model
    model = AutoformerModel(lookback=LOOKBACK, num_features=X_train_scaled.shape[1], horizon=HORIZON, d_model=128, num_heads=8, d_ff=512, num_layers=2, dropout_rate=0.1).to(device)
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    results_data["total_parameters"] = total_params

    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=0.0006660688102168459, weight_decay=4.931497482935789e-06)
    ...
    if best_val_loss < best_overall_val_loss and best_model_weights is not None:
        ...
        best_seed_id = SEED
        torch.save(best_model_weights, f"03_tfm_afm_pytorch_best.pt")
        results_data["best_seed"] = int(SEED)
        print(f"  [Checkpoint] New overall best model saved from SEED {SEED} (Val Loss: {best_val_loss:.6f}) -> 03_tfm_afm_pytorch_best.pt")
...
np.savez_compressed(f"03_tfm_afm_pytorch_predictions.npz", **all_predictions)
print(f"Successfully saved all seed predictions to 03_tfm_afm_pytorch_predictions.npz")

print(f"\n======================================================================")
print(f"FINAL SUMMARY ACROSS {len(SEEDS)} SEEDS — 03_tfm_afm_pytorch")
print(f"======================================================================")
summary_dict = {}
