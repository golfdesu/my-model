...
# # LSTM Baseline (Encoder-only, Direct Multi-Step Head)
# Deep learning baseline WITHOUT attention - mirrors 01_tfm_tfm_pytorch.py
# (Baseline Vanilla Transformer) exactly except the core sequence layer is swapped
# from nn.TransformerEncoder to nn.LSTM. This isolates "attention vs recurrence"
# as the single variable being compared.
...
# Helper 2: PyTorch Gaussian Noise Layer (kept identical to Baseline Transformer for fair comparison)
...
# Helper 3: LSTM Encoder-Only Architecture in PyTorch
# Mirrors EncoderOnlyTransformer (01_tfm_tfm_pytorch.py) exactly, except
# nn.TransformerEncoder -> nn.LSTM (no attention, no positional embedding needed
# since LSTM is inherently sequential/order-aware).
class LSTMBaseline(nn.Module):
    def __init__(self, lookback, num_features, horizon, d_model=64, num_layers=2,
        ...
        # Core sequence layer: LSTM instead of Transformer encoder.
        # dropout applies between LSTM layers only when num_layers > 1 (PyTorch convention).
        self.lstm = nn.LSTM(
            input_size=d_model,
            hidden_size=d_model,
        ...
        # Head: identical structure to EncoderOnlyTransformer's head
        # (dual aggregation: last-step + global average pooling)
    ...
    def forward(self, x):
        ...
        lstm_out, _ = self.lstm(x)  # [batch, lookback, d_model]

        # Dual Feature Aggregation (Last step + Global Average Pooling) - same as Transformer baseline
...
results_data = {
    "model_name": "09_lstm_baseline_pytorch",
    "seeds": {},
    "summary": {}
}
all_seed_metrics = []
...
def run_benchmark():
    global results_data, all_seed_metrics, all_predictions, best_overall_val_loss, best_seed_id
    ...
    print(f"Starting Automated {len(SEEDS)}-Seed Loop for 09_lstm_baseline_pytorch in PyTorch...")
    ...
    for seed_idx, SEED in enumerate(SEEDS, 1):
        ...
        # Build Model
        model = LSTMBaseline(lookback=LOOKBACK, num_features=X_train_scaled.shape[1], horizon=HORIZON, d_model=32, num_layers=2, dropout_rate=0.05, noise_stddev=0.01).to(device)
        total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        results_data["total_parameters"] = total_params

        criterion = nn.MSELoss()
        optimizer = optim.Adam(model.parameters(), lr=0.0002451331674531841, weight_decay=5.590664095943572e-06)
        ...
        if best_val_loss < best_overall_val_loss and best_model_weights is not None:
            ...
            best_seed_id = SEED
            torch.save(best_model_weights, f"09_lstm_baseline_pytorch_best.pt")
            results_data["best_seed"] = int(SEED)
            print(f"  [Checkpoint] New overall best model saved from SEED {SEED} (Val Loss: {best_val_loss:.6f}) -> 09_lstm_baseline_pytorch_best.pt")
    ...
    np.savez_compressed(f"09_lstm_baseline_pytorch_predictions.npz", **all_predictions)
    print(f"Successfully saved all seed predictions to 09_lstm_baseline_pytorch_predictions.npz")

    print(f"\n======================================================================")
    print(f"FINAL SUMMARY ACROSS {len(SEEDS)} SEEDS — 09_lstm_baseline_pytorch")
    print(f"======================================================================")
    summary_dict = {}
