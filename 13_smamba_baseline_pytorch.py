...
print(f"Starting Automated {len(SEEDS)}-Seed Loop for 13_smamba_baseline_pytorch in PyTorch...")
...
for seed_idx, SEED in enumerate(SEEDS, 1):
    ...
    model = SMambaModel(lookback=LOOKBACK, num_features=X_train_scaled.shape[1], horizon=HORIZON, d_model=32, d_state=32, num_layers=2, dropout_rate=0.3, target_idx=TARGET_CH_IDX).to(device)
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    results_data["total_parameters"] = total_params
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=0.004943546841200545, weight_decay=5.544295449317336e-06)
...
np.savez_compressed(f"13_smamba_baseline_pytorch_predictions.npz", **all_predictions)
print(f"Successfully saved all seed predictions to 13_smamba_baseline_pytorch_predictions.npz")

print(f"\n======================================================================")
print(f"FINAL SUMMARY ACROSS {len(SEEDS)} SEEDS — 13_smamba_baseline_pytorch")
print(f"======================================================================")
summary_dict = {}
...
print(f"\nFinished running all {len(SEEDS)} SEEDs for 13_smamba_baseline_pytorch!")
