...
for seed_idx, SEED in enumerate(SEEDS, 1):
    ...
    if best_val_loss < best_overall_val_loss and best_model_weights is not None:
        ...
        best_seed_id = SEED
        torch.save(best_model_weights, f"07_tfm_itfm_pytorch_best.pt")
        results_data["best_seed"] = int(SEED)
        print(f"  [Checkpoint] New overall best model saved from SEED {SEED} (Val Loss: {best_val_loss:.6f}) -> 07_tfm_itfm_pytorch_best.pt")
...
np.savez_compressed(f"07_tfm_itfm_pytorch_predictions.npz", **all_predictions)
print(f"Successfully saved all seed predictions to 07_tfm_itfm_pytorch_predictions.npz")

print(f"\n======================================================================")
print(f"FINAL SUMMARY ACROSS {len(SEEDS)} SEEDS — 07_tfm_itfm_pytorch")
print(f"======================================================================")
