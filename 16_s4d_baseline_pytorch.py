...
for seed_idx, SEED in enumerate(SEEDS, 1):
    ...
    model = S4DModel(lookback=LOOKBACK, num_features=X_train_scaled.shape[1], horizon=HORIZON, d_model=128, d_state=128, num_layers=3, dropout_rate=0.25).to(device)
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    results_data["total_parameters"] = total_params

    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=0.004702937366435549, weight_decay=9.949325040041723e-05)
