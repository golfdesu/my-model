...
output_json_filename = "05_tfm_afm_pytorch_results.json"
results_data = {
    "model_name": "05_tfm_afm_pytorch",
    "seeds": {},
    "summary": {}
}
all_seed_metrics = []
...
print(f"Starting Automated {len(SEEDS)}-Seed Loop for 05_tfm_afm_pytorch in PyTorch...")
...
for seed_idx, SEED in enumerate(SEEDS, 1):
    ...
    # Build Model
    model = AutoformerModel(lookback=LOOKBACK, num_features=X_train_scaled.shape[1], horizon=HORIZON, d_model=64, num_heads=2, d_ff=256, num_layers=3, dropout_rate=0.15).to(device)
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    results_data["total_parameters"] = total_params

    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=0.0008158150440167179, weight_decay=1.4421210830446152e-05)
    ...
    if best_val_loss < best_overall_val_loss and best_model_weights is not None:
        ...
        best_seed_id = SEED
        torch.save(best_model_weights, f"05_tfm_afm_pytorch_best.pt")
        results_data["best_seed"] = int(SEED)
        print(f"  [Checkpoint] New overall best model saved from SEED {SEED} (Val Loss: {best_val_loss:.6f}) -> 05_tfm_afm_pytorch_best.pt")
...
np.savez_compressed(f"05_tfm_afm_pytorch_predictions.npz", **all_predictions)
print(f"Successfully saved all seed predictions to 05_tfm_afm_pytorch_predictions.npz")

print(f"\n======================================================================")
print(f"FINAL SUMMARY ACROSS {len(SEEDS)} SEEDS — 05_tfm_afm_pytorch")
print(f"======================================================================")
summary_dict = {}
