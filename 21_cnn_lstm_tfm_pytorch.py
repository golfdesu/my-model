...
# ==============================================================================
# Model 21: CNN-LSTM-Transformer Hybrid PyTorch Implementation
...
#    Extracts short-term temporal trends, sharp ramping rates, and high-frequency local
#    patterns across all exogenous multivariate time series without reducing sequence length.
# 2. Recurrent LSTM Sequence Context:
#    Processes CNN feature maps sequentially to maintain recurrent temporal flow and hidden state memory.
# 3. Multi-Head Self-Attention Transformer:
#    Applies multi-head scaled dot-product self-attention across the sequence tokens to capture
#    long-range periodic daily and multi-day correlations.
# 4. Direct Multi-Step Projection Head:
#    Linear projection forecasting the full H=48 future horizon (24 hours at 30-min intervals)
#    simultaneously, avoiding autoregressive error accumulation.
#
...
# ---------------------------------------------------------
# 3. Model Architecture: CNN-LSTM-Transformer Hybrid
# ---------------------------------------------------------
...
class CNNLSTMTransformer(nn.Module):
    def __init__(
        ...
        # 1. 1D CNN Local Feature Extractor
        # Input: [B, C_in=num_features, L=lookback]
        ...
        # 2. Recurrent LSTM Sequence Context
        # Input to LSTM: [B, L, cnn_channels]
        self.lstm = nn.LSTM(
        ...
        # 4. Direct Multi-Step Projection Head
        # Aggregates sequence features (last step + mean pooling) into final horizon
    ...
    def forward(self, x):
        ...
        x_cnn = x_cnn.transpose(1, 2)  # [B, L, cnn_channels]

        # 2. LSTM Sequence Modeling
        lstm_out, _ = self.lstm(x_cnn)  # [B, L, lstm_hidden]

        # 3. Transformer Multi-Head Self-Attention
        tfm_in = self.proj_to_tfm(lstm_out)  # [B, L, d_model]
        tfm_in = self.pos_encoder(tfm_in)
        tfm_out = self.transformer_encoder(tfm_in)  # [B, L, d_model]

        # 4. Aggregation and Projection Head
        last_step = tfm_out[:, -1, :]              # [B, d_model]
        mean_pool = tfm_out.mean(dim=1)            # [B, d_model]
        fused = torch.cat([last_step, mean_pool], dim=-1)  # [B, 2 * d_model]
...
MODEL_NAME = "21_cnn_lstm_tfm_pytorch"
...
CONFIG = {
    ...
    "tfm_layers": 1,
    "dropout": 0.2,
    "learning_rate": 0.001493155292930546,
    "weight_decay": 0.0002682844682308789,
    "batch_size": 128,
    "epochs": 100,
    "patience": 12}
