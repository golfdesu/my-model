# Baseline Transformer Encoder in PyTorch (Vanilla Transformer - Vaswani et al., NIPS 2017)
Converted from TensorFlow/Keras implementation (`01_tfm_tfm_auto.ipynb`) to PyTorch with Colab GPU Auto-Detect, `tqdm` Progress Bar & CPU Optimizations.
...
`	ext
Dataset Loaded successfully from acn_caltech_ready2.csv! Total Rows: 32435, Features Count: 27
`
...
`	ext
Peak Load Threshold (Top 20% of TRAIN): 34.7101 kW
`
...
def create_windowed_dataset_pytorch(X_data, y_data, lookback, horizon, batch_size=64, shuffle=True, drop_last=None):
    ...
    if drop_last is None:
        drop_last = shuffle
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, drop_last=drop_last, num_workers=2, pin_memory=True, persistent_workers=True if drop_last else False)
...
class EncoderOnlyTransformer(nn.Module):
    def __init__(self, lookback, num_features, horizon, d_model=128, num_heads=8, d_ff=256, num_layers=3,
                 dropout_rate=0.1, noise_stddev=0.05):
        super().__init__()
...
# Helper 5: Metrics Evaluator Function
...
for seed_idx, SEED in enumerate(SEEDS, 1):
    ...
    train_loader, _, _ = create_windowed_dataset_pytorch(X_train_scaled, y_train_scaled, LOOKBACK, HORIZON, BATCH_SIZE, shuffle=True)
    val_loader, _, _   = create_windowed_dataset_pytorch(X_val_scaled, y_val_scaled, LOOKBACK, HORIZON, BATCH_SIZE, shuffle=False)
    test_loader, X_test_seq, y_test_seq = create_windowed_dataset_pytorch(X_test_scaled, y_test_scaled, LOOKBACK, HORIZON, BATCH_SIZE, shuffle=False)

    # Build Model & Enforce Mandatory torch.compile() with C++ Compiler (cl.exe)
    model = EncoderOnlyTransformer(lookback=LOOKBACK, num_features=X_train_scaled.shape[1], horizon=HORIZON, d_model=128, num_heads=8, d_ff=256, num_layers=3, dropout_rate=0.1).to(device)
    # Native PyTorch execution (bypassing MSVC cl.exe compilation overhead on Windows CPU)
...
`	ext
Starting Automated 1-Seed Loop for 01_tfm_tfm_pytorch.md in PyTorch...

=========================================================================
RUNNING SEED 164 (1/1)
=========================================================================
`

`	ext
Seed 164 Training:   0%|          | 0/100 [00:00<?, ?it/s]
`
