...
output_json_filename = "02_tfm_ifm_pytorch_bf16_results.json"
results_data = {
    "model_name": "02_tfm_ifm_pytorch_bf16",
...
print(f"Starting Automated {len(SEEDS)}-Seed Loop for 02_tfm_ifm_pytorch_bf16 in PyTorch...")
...
for seed_idx, SEED in enumerate(SEEDS, 1):
    ...
    with torch.inference_mode():
        for batch_X, _ in test_loader:
            ...
            with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=(device.type == 'cuda')):
                out = model(batch_X)
            y_pred_list.append(out.float().cpu().numpy())
    ...
    if best_val_loss < best_overall_val_loss and best_model_weights is not None:
        ...
        best_seed_id = SEED
        torch.save(best_model_weights, f"02_tfm_ifm_pytorch_bf16_best.pt")
        results_data["best_seed"] = int(SEED)
        print(f"  [Checkpoint] New overall best model saved from SEED {SEED} (Val Loss: {best_val_loss:.6f}) -> 02_tfm_ifm_pytorch_bf16_best.pt")
...
np.savez_compressed(f"02_tfm_ifm_pytorch_bf16_predictions.npz", **all_predictions)
print(f"Successfully saved all seed predictions to 02_tfm_ifm_pytorch_bf16_predictions.npz")

print(f"\n======================================================================")
print(f"FINAL SUMMARY ACROSS {len(SEEDS)} SEEDS — 02_tfm_ifm_pytorch_bf16")
print(f"======================================================================")
summary_dict = {}
